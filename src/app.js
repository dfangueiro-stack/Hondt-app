...
function updateScenarioSummary() {
  scenarioSummary.textContent = `${state.partyCount} lists · ${state.seatCount} seats`;
}
...
function collectParties() {
  ...
  for (let index = 0; index < state.partyCount; index += 1) {
    const name = String(formData.get(`partyName-${index}`) ?? "").trim() || `Party ${index + 1}`;
...
function calculateDHondt(parties, seatCount) {
  ...
  quotients.sort((left, right) => {
    ...
    return left.partyName.localeCompare(right.partyName, "en");
  });
...
function renderQuotientTable(parties, seatCount) {
  ...
  quotientMeta.textContent = `${seatCount} columns`;
}
...
function renderResults(parties, selected) {
  ...
  totalVotes.textContent = `${formatNumber(total)} total votes`;
...
function populateDemoData() {
  ...
  for (let index = 0; index < state.partyCount; index += 1) {
    ...
    nameInput.value = `List ${String.fromCharCode(65 + (index % 26))}${index >= 26 ? index + 1 : ""}`;
...
function buildPdfDocument() {
  ...
  const scenario = `${state.partyCount} lists · ${state.seatCount} seats`;
...
function exportPdfSummary() {
  ...
  if (!frameDocument || !printFrame.contentWindow) {
    printFrame.remove();
    window.alert("It was not possible to prepare the PDF export in this browser.");
    return;
  }
...
setupForm.addEventListener("submit", (event) => {
  ...
  totalVotes.textContent = "0 total votes";
  quotientTable.innerHTML = "";
  quotientMeta.textContent = "0 columns";
